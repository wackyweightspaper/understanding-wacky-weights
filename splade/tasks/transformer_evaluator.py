...
class SparseRetrieval(Evaluator):
    ...
    def numba_score_float(inverted_index_ids: numba.typed.Dict,
        ...
        for _idx in range(n):
            ...
            # Check if the token exists in the inverted index
            if local_idx not in inverted_index_ids:
                continue
    ...
    def retrieve(self, q_loader, top_k, name=None, return_d=False, id_dict=False, threshold=0):
        ...
        with torch.no_grad():
            ...
            for t, batch in enumerate(tqdm(q_loader)):
                ...
                encoded_queries.append((q_id, col.cpu().numpy(), values.cpu().numpy().astype(np.float32)))
            ...
            # For debugging purposes
            with open(os.path.join(self.out_dir, "encoded_queries.pkl"), "wb") as f:
