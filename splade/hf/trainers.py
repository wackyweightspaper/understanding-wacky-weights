...
class BaseTrainer(Trainer):
    ...
    def _flops(inputs):
        # DF-FLOPS: sum_t ( w_t * (1/N * sum_i r_{i,t}) )^2
        # Approximate DF_t/|C| with batch non-zero ratio per term.
        batch_mean = torch.mean(torch.abs(inputs), dim=0)
        df_ratio = torch.mean((inputs != 0).float(), dim=0)
        w_t = torch.sigmoid(df_ratio)
        return torch.sum((w_t * batch_mean) ** 2)
