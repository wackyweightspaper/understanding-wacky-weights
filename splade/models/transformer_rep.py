...
import torch
from transformers import AutoTokenizer, AutoModelForMaskedLM, AutoModel, AutoConfig, BertForMaskedLM
...
class TransformerRep(torch.nn.Module):
    def __init__(self, model_type_or_dir, output, fp16=False, early_exit_layer=None):
        """
        ...
        #     self.config.num_hidden_layers = early_exit_layer
        #     self.transformer = BertForMaskedLM(self.config)
        #     transformer_full = BertForMaskedLM.from_pretrained(model_type_or_dir)
        #     self.transformer.load_state_dict(transformer_full.state_dict(), strict=False)
        # else:
        #     self.transformer = model_class.from_pretrained(model_type_or_dir)
        
        self.transformer = model_class.from_pretrained(model_type_or_dir)
        if early_exit_layer is not None:
            self.transformer.bert.encoder.layer = torch.nn.ModuleList(
                list(self.transformer.bert.encoder.layer[:early_exit_layer])
            )
    ...
    #             # Prepare extended attention mask (same as BERT internal logic)
    #             # Shape: [batch_size, 1, 1, seq_len], where 0.0 = keep, -10000.0 = mask
    #             extended_attention_mask = tokens['attention_mask'][:, None, None, :].to(dtype=torch.float)
    #             extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
                
    #             out = self.transformer.bert.embeddings(tokens['input_ids'])
    #             for i, layer_module in enumerate(self.transformer.bert.encoder.layer):
    #                 if i == self.early_exit_layer:
    #                     break
    #                 out = layer_module(out)[0]
    #             out = {"logits": self.transformer.cls(out)}
    #         else:
    ...
    #             intermediate_output["hidden_states"] = output[0]
    #         return hook_fn
    ...
    #                 hook_handle = self.transformer.embeddings[self.early_exit_layer-1].register_forward_hook(
    #                     get_hook(self.early_exit_layer)
    #                 )
    #             else:
    #                 hook_handle = self.transformer.bert.encoder.layer[self.early_exit_layer-1].register_forward_hook(
    #                     get_hook(self.early_exit_layer)
    #                 )
    #             _ = self.transformer(**tokens)
    #             hook_handle.remove()
    #             out = {"logits": self.transformer.cls(intermediate_output["hidden_states"])}
    #         else:
    #             out = self.transformer(**tokens)
...
class SiameseBase(torch.nn.Module, ABC):
    ...
    def __init__(self, model_type_or_dir, output, match="dot_product", model_type_or_dir_q=None, freeze_d_model=False,
                 fp16=False, early_exit_layer=None):
        super().__init__()
        self.output = output
        ...
        self.match = match
        self.fp16 = fp16
        self.transformer_rep = TransformerRep(model_type_or_dir, output, fp16, early_exit_layer=early_exit_layer)
...
class Splade(SiameseBase):
    ...
    def encode(self, tokens, is_q):
        ...
        if self.agg == "sum":
            ...
            values = (torch.log(1 + torch.relu(out[:, 0, :])))
            return values
        else:
...
class SpladeTopK(SiameseBase):
    ...
    def __init__(self, model_type_or_dir, model_type_or_dir_q=None,
                 freeze_d_model=False, agg="max", fp16=False, output='MLM', top_d=32, top_q=5, early_exit_layer=None, **kwargs):
        super().__init__(model_type_or_dir=model_type_or_dir,
                         output=output,
                         match='dot_product',
                         model_type_or_dir_q=model_type_or_dir_q,
